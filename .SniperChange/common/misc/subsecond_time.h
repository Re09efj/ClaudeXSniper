...
public:
   ...
   friend inline void atomic_set_subsecondtime(SubsecondTime &dest, const SubsecondTime &src);
   friend inline SubsecondTime atomic_get_subsecondtime(const SubsecondTime &src);
   friend inline void atomic_update_max_subsecondtime(SubsecondTime &dest, const SubsecondTime &src);
...
// 2026-07-12: ShmemPerfModel::setElapsedTime()/getElapsedTime()/
// updateElapsedTime() had their locking (m_shmem_perf_model_lock) commented
// out entirely, presumably for performance (this is a very hot per-memory-
// access path) -- only incrElapsedTime()/incrTotalMemoryAccessLatency() were
// made safe via atomic_add_subsecondtime() above. ThreadSanitizer confirmed
// a genuine unprotected race on m_elapsed_time during the LU/2TH/W silent-
// hang investigation (see Documents/SniperBugFix.md), reached via
// CacheCntlr::updateCacheBlock()'s cross-controller sibling-cache invalidate
// path (the same architecture pattern as the cache_block_info.h fix -- each
// core's ShmemPerfModel is touched directly from another core's thread
// without going through that core's own lock). These three helpers extend
// the same "atomic op directly on m_time" style already used above, rather
// than introducing a different synchronization mechanism.
inline void atomic_set_subsecondtime(SubsecondTime &dest, const SubsecondTime &src)
{
   __atomic_store_n(&dest.m_time, src.m_time, __ATOMIC_SEQ_CST);
}

inline SubsecondTime atomic_get_subsecondtime(const SubsecondTime &src)
{
   SubsecondTime result;
   result.m_time = __atomic_load_n(const_cast<uint64_t*>(&src.m_time), __ATOMIC_SEQ_CST);
   return result;
}

inline void atomic_update_max_subsecondtime(SubsecondTime &dest, const SubsecondTime &src)
{
   uint64_t old_val = __atomic_load_n(&dest.m_time, __ATOMIC_SEQ_CST);
   while (src.m_time > old_val)
   {
      if (__atomic_compare_exchange_n(&dest.m_time, &old_val, src.m_time, false, __ATOMIC_SEQ_CST, __ATOMIC_SEQ_CST))
         break;
      // On failure, old_val is updated to the current value by compare_exchange_n; loop retries.
   }
}
