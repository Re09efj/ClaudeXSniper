...
class TraceManager
{
   private:
      ...
      // 2026-07-12: signalStarted() (called by each new TraceThread) increments
      // this with a plain "++", unlike signalDone() right below it in
      // trace_manager.cc which takes m_lock. Monitor::run() polls this
      // unlocked in a tight startup-wait loop. ThreadSanitizer confirmed this
      // as a genuine, unprotected data race (see Documents/SniperBugFix.md) --
      // looks like a straightforward oversight (signalDone was clearly written
      // with thread-safety in mind; signalStarted wasn't). Only 3 use sites
      // (init, ++, and a > 0 read), all atomic-op-compatible, so std::atomic
      // is a minimal, low-risk fix here.
      std::atomic<UInt32> m_num_threads_started;
