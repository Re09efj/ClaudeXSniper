...
void TraceManager::signalDone(TraceThread *thread, SubsecondTime time, bool aborted)
{
   ...
   // If every remaining thread of this application is currently stalled (blocked on a
   // futex), none of them can ever send each other a wake -- this is an app-scoped
   // deadlock, most commonly because a sibling's trace ended (cleanly or via a crashed
   // Pin recorder) without ever recording the final wake syscall a still-waiting sibling
   // needed. Force-resume all of them defensively so they notice end-of-trace / m_stop
   // instead of hanging forever.
   // 2026-07-06: root-caused via a fluidanimate hang -- a sibling thread's trace can reach
   // a clean RecOtherEnd (printed as "-- DONE --", not "-- STOP --") without ever recording
   // its final wake syscall for this thread, if the underlying Pin process crashed while
   // flushing that sibling's very last events (the trailing signal-11 already known from
   // GAPBS). endApplication() already does this for the exit_group/abort path; this handles
   // the normal-completion path too, since both funnel through signalDone().
   // 2026-07-12: generalized from the original "exactly one thread left" check (which only
   // covers the last-straggler case) to "all remaining threads are stalled" so that
   // multi-stage pipelines (e.g. dedup, where several concurrent stages can be left stuck
   // on one dead producer while OTHER, unrelated app threads keep running normally -- so
   // num_threads never drops to 1) are also rescued. This is still safe: we only ever
   // force-resume a thread when there is no other running sibling left that could still
   // send it a legitimate wake.
   if (m_app_info[app_id].num_threads > 0)
   {
      ScopedLock sl2(Sim()->getThreadManager()->getLock());
      bool all_stalled = true;
      for (std::vector<TraceThread *>::iterator it = m_threads.begin(); it != m_threads.end() && all_stalled; ++it)
      {
         if ((*it)->getThread()->getAppId() == app_id && !(*it)->m_stopped)
         {
            if (Sim()->getThreadManager()->getThreadState((*it)->getThread()->getId()) != Core::STALLED)
               all_stalled = false;
         }
      }
      if (all_stalled)
      {
         for (std::vector<TraceThread *>::iterator it = m_threads.begin(); it != m_threads.end(); ++it)
         {
            if ((*it)->getThread()->getAppId() == app_id && !(*it)->m_stopped)
            {
               Sim()->getThreadManager()->resumeThread((*it)->getThread()->getId(), thread->getThread()->getId(), time);
            }
         }
      }
   }
...
void TraceManager::endApplication(TraceThread *thread, SubsecondTime time)
{
   for(std::vector<TraceThread *>::iterator it = m_threads.begin(); it != m_threads.end(); ++it)
   {
      ...
      if ((*it)->getThread()->getAppId() == thread->getThread()->getAppId() && *it != thread)
      {
         // Ask thread to stop
         (*it)->stop();
         // Threads are often blocked on a futex in this case (waiting for a FUTEX_WAKE that
         // was supposed to come from a sibling thread's trace, which will now never arrive
         // since that sibling's application just called exit_group()). Setting the m_stop
         // flag alone does not unblock a thread parked inside ThreadManager::stallThread(),
         // so force-resume it here; its TraceThread::run() loop will then notice m_stop
         // (or Sift::Reader::Read() will simply hit end-of-trace) and exit cleanly instead
         // of hanging forever. 2026-07-06: root-caused via fluidanimate hang investigation.
         {
