   ...
   , _ignore_local(Sim()->getCfg()->getBool("network/bus/ignore_local_traffic"))
{
   ...
   // 2026-07-10: 従来はpkt.sender==pkt.receiver(数値完全一致)の時だけバス課金を
   // 免除していたが、これだとコントローラの代表コア番号と一致する場合しか
   // 免除されず、「同じノード内の他コアからのオンチップアクセス」が免除
   // されない欠陥があった(Jinのコメント「Memory controllers are on-chip, so
   // traffic from core0 to dram0 does not use the QPI links」の意図を正しく
   // 再現できていなかった)。general/cores_per_nodeでノード境界を読み、
   // 同ノードかどうかで免除するよう修正。未設定の他configを壊さないよう、
   // 読めなければ「全コアが1ノード」にフォールバックする。
   try
   {
      _cores_per_node = Sim()->getCfg()->getInt("general/cores_per_node");
      if (_cores_per_node == 0)
         _cores_per_node = Config::getSingleton()->getApplicationCores();
   }
   catch (...)
   {
      _cores_per_node = Config::getSingleton()->getApplicationCores();
...
NetworkModelBus::accountPacket(const NetPacket &pkt)
{
   ...
   if (  !_enabled
         || (_ignore_local && (pkt.sender / (SInt32)_cores_per_node) == (pkt.receiver / (SInt32)_cores_per_node))
