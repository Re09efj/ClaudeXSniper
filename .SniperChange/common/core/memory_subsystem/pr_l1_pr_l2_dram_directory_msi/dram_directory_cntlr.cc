      ...
      core_id_t receiver, IntPtr address, Byte* cached_data_buf, ShmemMsg *orig_shmem_msg)
{
   ...
   else
   {
      ...
      // 2026-07-10 First-Touch: orig_shmem_msgの元々のrequester(実際にメモリを
      // 要求したアプリケーションコア)を渡す。
      core_id_t dram_node = m_dram_controller_home_lookup->getHome(address, orig_shmem_msg->getRequester());
...
DramDirectoryCntlr::processDRAMReply(core_id_t sender, ShmemMsg* shmem_msg)
{
   ...
   {
      // 2026-07-10: 元は sender==requester(コア番号の完全一致)という判定だった
      // ため、同じノード内でもコア番号が異なるだけでREMOTE扱いになるバグが
      // あった(network_model_bus.cc:accountPacket()と同系統の問題)。
      // general/cores_per_nodeでノード境界を読み、ノード単位の比較に修正する。
      ...
      bool same_node = (sender / (core_id_t)s_cores_per_node) == (requester / (core_id_t)s_cores_per_node);
      hit_where = same_node ? HitWhere::DRAM_LOCAL : HitWhere::DRAM_REMOTE;
      static long dbgcount=0; dbgcount++;
      if (dbgcount % 5000 == 0) { printf("[GDBG] n=%ld cpn=%u sender=%d requester=%d same=%d\n", dbgcount, s_cores_per_node, sender, requester, (int)same_node); }
   }
...
DramDirectoryCntlr::sendDataToNUCA(IntPtr address, core_id_t requester, Byte* data_buf, SubsecondTime now, bool count)
{
   if (m_nuca_cache)
   {
      ...
      {
         // Write data to Dram
         // 2026-07-10 First-Touch: evict_addressは既にfirst-touch記録済みのはず
         // (以前アクセスされてキャッシュに乗ったから追い出される)なので、この
         // requesterは通常map hit時は使われない。安全側でrequesterを渡す。
...
DramDirectoryCntlr::sendDataToDram(IntPtr address, core_id_t requester, Byte* data_buf, SubsecondTime now)
{
   ...
   {
      // Write data to Dram
      // 2026-07-10 First-Touch: requesterを渡す。
