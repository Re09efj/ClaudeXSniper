   ...
   class CacheCntlr : ::CacheCntlr
   {
      private:
         ...
         struct {
           ...
           // 2026-07-10: 「本棚問題」検証用の新計測ポイント。initiateDirectoryAccess()で
           // 要求元コア(m_core_id_master、本物のアプリケーションコア)とgetHome(address)
           // (First-Touchで決まる持ち主)のノードを直接比較する。既存のloads_where[DRAM_LOCAL/
           // REMOTE]はタグディレクトリ⇔DRAMコントローラという内部プロトコル区間しか見ておらず、
           // First-Touch後は常に一致してしまうため、アプリケーションスレッド〜データ間の
           // 真のローカリティを測れなかった(詳細はDocuments/2026年7月10日.md参照)。
           UInt64 access_home_local, access_home_remote;
         ...
         // 2026-07-10: true_requester(本物の要求元コア)を追加。「本棚問題」検証用の
         // access-home-local/remote計測に、共有L3のm_core_id_master(代表コア、常に
         // ノード境界と一致してしまうため使えない)ではなく、本物の要求元コアを渡す。
         void initiateDirectoryAccess(Core::mem_op_t mem_op_type, IntPtr address, bool isPrefetch, SubsecondTime t_issue, core_id_t true_requester);
         ...
         // 2026-07-10 First-Touch: 要求元コア(このキャッシュコントローラ自身の
         // コア)を渡し、初回アクセス時のノードにホームを固定する。
         core_id_t getHome(IntPtr address) { return m_tag_directory_home_lookup->getHome(address, m_core_id_master); }
