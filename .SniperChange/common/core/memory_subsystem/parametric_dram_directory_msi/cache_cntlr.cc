   ...
   m_shmem_perf_model(shmem_perf_model)
{
   ...
   // 2026-07-10: 「本棚問題」検証用。initiateDirectoryAccess()参照。
   registerStatsMetric(name, core_id, "access-home-local", &stats.access_home_local);
   registerStatsMetric(name, core_id, "access-home-remote", &stats.access_home_remote);
...
CacheCntlr::processShmemReqFromPrevCache(CacheCntlr* requester, Core::mem_op_t mem_op_type, IntPtr address, bool modeled, bool count, Prefetch::prefetch_type_t isPrefetch, SubsecondTime t_issue, bool have_write_lock)
{
   ...
   else // !cache_hit: either data is not here, or operation on data is not permitted
   {
      ...
      else // last-level cache
      {
         ...
         {
            // 2026-07-10: requester(本物の要求元CacheCntlr、L1相当)のコアIDを
            // true_requesterとして渡す(access-home-local/remote計測用)。
            initiateDirectoryAccess(mem_op_type, address, isPrefetch != Prefetch::NONE, t_issue,
                                     requester ? requester->m_core_id : INVALID_CORE_ID);
         }
      }
   }
...
void
CacheCntlr::initiateDirectoryAccess(Core::mem_op_t mem_op_type, IntPtr address, bool isPrefetch, SubsecondTime t_issue, core_id_t true_requester)
{
   ...
   if (first)
   {
      ...
      // 2026-07-10 「本棚問題」検証用の新計測ポイント。true_requester(呼び出し元の
      // processShmemReqFromPrevCache()が持つ本物の要求元CacheCntlrのコア、L1相当)を
      // 使う。m_core_id_masterは共有L3の代表コア(m_core_id - m_core_id%shared_cores)
      // でしかなく、常にノード境界と一致してしまうため使えない。
      if (true_requester != INVALID_CORE_ID)
      {
         static UInt32 s_cores_per_node = 0;
         if (s_cores_per_node == 0)
         {
            try { s_cores_per_node = Sim()->getCfg()->getInt("general/cores_per_node"); }
            catch (...) { s_cores_per_node = 0; }
            if (s_cores_per_node == 0)
               s_cores_per_node = Config::getSingleton()->getApplicationCores();
         }
         core_id_t home = getHome(address);
         bool same_node = ((core_id_t)true_requester / (core_id_t)s_cores_per_node)
                        == (home / (core_id_t)s_cores_per_node);
         if (same_node)
            stats.access_home_local++;
         else
            stats.access_home_remote++;
      }
