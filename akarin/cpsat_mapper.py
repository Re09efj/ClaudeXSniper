...
単体実行:
  python3 -m utility.cpsat_mapper Data/tsuushin/sizeA/BT_A_12TH_bt.A.x.12.6.comm.csv --threads 12 --alpha 0.5
"""
...
from config.generate_config import P_FREQ, E_FREQ
...
# 追従できておらず、比4.0倍という全く違う前提でAKARINのCP-SAT最適化(alpha込みの
# makespan項)を解いていたことが発覚(他セッションからの指摘で判明)。実際に
# Sniperでシミュレートされる周波数とAKARINが最適化に使う周波数が異なると、
# CP-SATが求めるalphaの意味合い(ヘテロ性の重み)自体が実測とズレるため、
# 単一の真実源(generate_config.py)からimportする形に変更した。
